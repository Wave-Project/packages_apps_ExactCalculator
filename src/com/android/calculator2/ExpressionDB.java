...
public class ExpressionDB {
    ...
    private boolean notInAccessibleRange(long index) {
        if (!CONTINUE_WITH_BAD_DB) {
            return false;
        }
        synchronized (mLock) {
    ...
    public long addRow(boolean negativeIndex, RowData data) {
        ...
        synchronized (mLock) {
            ...
            if (notInAccessibleRange(newIndex)) {
    ...
    private static class ExpressionDBHelper extends SQLiteOpenHelper {
